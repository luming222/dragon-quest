...
@export var charge_hit_radius := 1.35
@export var miss_vulnerable_time := 1.1
@export var knockback_strength := 6.5
...
@export var vulnerable_multiplier := 1.25
...
func _charge_prepare() -> void:
	...
	if _combat_manager:
		_combat_manager.call("open_clash_window", _player, windup + 0.15)
	_start_charge()
...
func _check_charge_hit() -> void:
	...
	if _combat_manager:
		_combat_manager.call("request_hitstop", 0.05, 0.1)
	_charging = false
	_set_highlight_state("none")
...
func _apply_knockback_from(source_pos: Vector3, strength: float) -> void:
	...
	_knockback_time = max(_knockback_time, 0.18)
...
func _cache_mesh_overrides() -> void:
	...
	for node in meshes:
		...
		telegraph.albedo_color = Color(1.0, 0.78, 0.35, 1.0)
		var charge := StandardMaterial3D.new()
		charge.shading_mode = BaseMaterial3D.SHADING_MODE_UNSHADED
		charge.albedo_color = Color(0.95, 0.42, 0.22, 1.0)
