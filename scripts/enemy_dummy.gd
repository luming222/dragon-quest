...
func _follow_player(delta: float) -> void:
	...
	if to_player.length() <= attack_range * 0.85:
...
func _try_attack() -> void:
	if _player == null or _combat_manager == null:
		return
...
func _attack_sequence() -> void:
	...
	if _combat_manager:
		_combat_manager.call("open_clash_window", _player, clash_window)
